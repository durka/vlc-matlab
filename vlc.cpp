...
enum Verb { INIT, RELEASE, OPEN, CLOSE, FRAME, PLAY, PAUSE } verbs;
...
void frame(libvlc_media_player_t*, double);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    switch (verb) {
        ...
        case FRAME:
            if (!check_args(nrhs, 3)) return;
            frame(unpack_pointer(libvlc_media_player_t, prhs[1]), mxGetPr(prhs[2])[0]);
            break;
        case PLAY:
            if (!check_args(nrhs, 2)) return;
...
void frame(libvlc_media_player_t *player, double ms)
{
